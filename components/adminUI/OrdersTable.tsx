...
export function OrdersTable({ initialOrders }: OrdersTableProps) {
  ...
  const handleStatusChange = async (orderId: string | any, status: OrderType["deliveryStatus"]) => {
    ...
    try {
      ...
      if (result.success) {
        setOrders(orders.map((order) => (String(order._id) === String(orderId) ? { ...order, deliveryStatus: status } : order)));
  ...
  return (
    <div className='space-y-4'>
            ...
            {paginatedOrders.length === 0 ? (
              ...
              paginatedOrders.map((order) => (
                <TableRow key={String(order._id)}>
                  <TableCell className='font-medium'>#{order.trackingNumber.substring(4, 12)}</TableCell>
                  <TableCell>
                    <div>
                  ...
                  <TableCell>
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground'>{formatDistanceToNow(new Date(order.createdAt || ""), { addSuffix: true })}</p>
                    </div>
                  </TableCell>
                  <TableCell>{renderStatusBadge(order.deliveryStatus)}</TableCell>
                  <TableCell>{renderPaymentBadge(order.paymentStatus)}</TableCell>
      ...
      {filteredOrders.length > 0 && <OrderPagination currentPage={currentPage} totalPages={totalPages} onPageChange={handlePageChange} />}
      ...
      <Sheet open={isDetailsOpen} onOpenChange={setIsDetailsOpen}>
        ...
        >
          ...
          <div className='px-6 py-6'>{selectedOrder && <OrderDetails order={selectedOrder} />}</div>
        </SheetContent>
      </Sheet>
    </div>
  );
