...
const CategoryFormSchema = z.object({
  ...
  icon: z
    ...
    .refine(
      ...
      "Icon must be a valid image file (JPEG, PNG, WebP, or SVG)",
    ),
  types: z.array(TypeSchema).min(1, "At least one type is required"),
})
...
export default function CategoryForm() {
  const [formData, setFormData] = useState<
    Omit<CategoryFormData, "icon" | "types"> & { icon: File | null; types: TypeFormItem[] }
  >({
    name: "",
    slug: "",
    icon: null,
  ...
  const updateType = (tempId: string, name: string) => {
    const newTypes = formData.types.map((type) =>
      type.tempId === tempId ? { ...type, name, slug: generateSlug(name) } : type,
    )
    setFormData((prev) => ({
      ...prev,
      types: newTypes,
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    console.log("Types Array:", submissionData.types)
    console.log("================================")
  ...
  const hasValidTypes = formData.types.length > 0 && formData.types.every((type) => type.name.trim())
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            {submitSuccess && (
              <Alert className="border-green-200 bg-green-50">
                ...
                <AlertDescription className="text-green-800">
                  Category created successfully! Check the console for form data.
                </AlertDescription>
              </Alert>
            )}
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <Input
                    ...
                    className={`cursor-pointer ${errors.icon ? "border-destructive" : ""}`}
                  />
                </div>
                {formData.icon && (
            ...
            {/* Form Summary */}
            {formData.name && formData.icon && formData.types.length > 0 && (
              <div className="bg-muted/50 p-4 rounded-lg">
                ...
                <div className="text-sm space-y-1">
                  ...
                  <p>
                    <span className="font-medium">Types:</span> {formData.types.filter((t) => t.name.trim()).length}{" "}
                    type(s)
                  </p>
                  <div className="mt-2">
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button type="submit" disabled={isSubmitting || !formData.name || !formData.icon || !hasValidTypes}>
