...
export default function ProductDetailsSection({ product }: ProductInfoProps) {
  ...
  const { _id, title, retailPrice, salePrice, saleInfo, images, brand, type, category, season, style, identifiers, slug, stock } = product;
        ...
        <div className='space-y-6'>
          ...
          <div className='space-y-0'>
            ...
            <p className='text-grey-darker'>
              {brand?.name || identifiers?.brand} {style} {type?.name || identifiers?.categoryType} {category?.name || identifiers?.category} - {season}
            </p>
          </div>
          <div className='space-y-6'>
            {/* Price and badges */}
            <div className='space-y-0'>
              ...
              {saleInfo && (
                <p className='italic text-[13px] text-success-green'>
                  <span className='line-through text-grey-darker'>{retailPrice?.toLocaleString("en-US")} MAD</span> saving {savedAmount?.toLocaleString("en-US")} MAD + Free
                  shipping
                </p>
              )}
            </div>
