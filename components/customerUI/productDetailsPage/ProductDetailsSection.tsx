...
export default function ProductDetailsSection({ product }: ProductInfoProps) {
  ...
  return (
    ...
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 pt-">
      ...
      <div className="space-y-4">
          ...
          {images.length > 1 && (
            <>
              <Button
                variant="ghost"
                size="icon"
                className="rounded-full absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/80 hover:bg-white"
                onClick={prevImage}
              >
              ...
              <Button
                variant="ghost"
                size="icon"
                className="rounded-full absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/80 hover:bg-white"
                onClick={nextImage}
              >
        ...
        <div className="flex overflow-x-auto gap-2">
          {images.map((img, index) => (
            <button
              key={index}
              className={`flex-shrink-0 w-16 h-16 bg-gray-50 rounded-xs overflow-hidden border-2 ${
                currentImageIndex === index ? "border-blue-500" : "border-transparent"
              }`}
              onClick={() => setCurrentImageIndex(index)}
            >
              <CldImage
                src={img.secure_url || "/placeholder-product.png"}
        ...
        <div className="space-y-6 pt-8">
          ...
          <div className="space-y-4">
            ...
            <div className="prose prose-sm max-w-none">
              <p>
                Experience ultimate safety and comfort with our Premium Sport Touring Motorcycle Helmet. Designed for
                adventure and long rides, this helmet features:
              </p>
          ...
          {/* Additional product images or content can go here */}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-0">
              ...
              <p className="text-grey-darker">
                {brand?.name || identifiers?.brand} {style} {type?.name || identifiers?.categoryType}{" "}
                {category?.name || identifiers?.category} - {season}
              </p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-0">
                <div className="flex gap-1 md:gap-2 lg:gap-3 items-center">
                  <span className="md:text-2xl text-3xl font-black tracking-wider text-blue-900">
                    {finalPrice?.toLocaleString("en-US")} MAD
                  </span>
                  {saleInfo && (
                    <>
                      <Badge className="text-white rounded-none bg-primary">
                        {saleInfo.discountType === "percentage"
                          ? `-${saleInfo.discountValue}%`
                          : `${saleInfo.discountValue} MAD`}
                      </Badge>
                ...
                {saleInfo && (
                  <p className="italic text-[13px] text-success-green">
                    <span className="line-through text-grey-darker">{retailPrice?.toLocaleString("en-US")} MAD</span>{" "}
                    saving {savedAmount?.toLocaleString("en-US")} MAD + Free shipping
                  </p>
                )}
              </div>
              ...
              <div className="p-4 space-y-4 custom-dashed">
                        ...
                        <button
                          ...
                          className={`px-3 py-2 border rounded-full text-sm font-medium transition-colors ${
                            selectedSize === sizeInfo.size
                              ...
                              : "border-grey-darker text-black hover:bg-grey"
                ...
                {(stock?.sizes?.length ?? 0) > 0 && (
                  <div>
                    ...
                    <div
                      className={`flex gap-3 items-center rounded-full border w-fit *:border-none *:bg-white/0 transition-opacity ${
          ...
          <div>
            <div className="flex flex-col gap-1">
              ...
              {selectedSize &&
                ...
                stock?.sizes?.find((s) => s.size === selectedSize)?.quantity === 0 && (
              ...
              <div className="flex gap-3">
                <Button
                  variant="outline"
                  size="lg"
                  className="flex-shrink-0 h-12 w-12 text-[16px] rounded-full border-grey-darker bg-transparent"
                >
