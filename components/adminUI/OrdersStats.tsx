...
export function OrdersStats({ orders }: OrdersStatsProps) {
  const stats = useMemo(() => {
    ...
    const paidRevenue = orders.filter((order) => order.paymentStatus === "paid").reduce((sum, order) => sum + order.orderTotalPrice, 0);
  ...
  return (
    <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
      <StatsCard title='Total Orders' value={stats.totalOrders.toString()} description='All time orders' icon={<CircleDollarSign className='size-4' />} />
      ...
      <StatsCard title='Shipping' value={stats.shipped.toString()} description='Orders in transit' icon={<Truck className='size-4' />} className='bg-amber-50 border-amber-100' />
      ...
      <StatsCard title='Cancelled' value={stats.cancelled.toString()} description='Cancelled orders' icon={<CircleX className='size-4' />} className='bg-red-50 border-red-100' />
  ...
  return (
    <Card className={`border ${className}`}>
      <CardContent className='p-4 flex items-center'>
        <div className='rounded-full bg-primary/10 p-2 mr-3'>{icon}</div>
        <div>
